...
def main():
    ...
    dataset_path = os.path.join(workspace_dir, "c_program_dataset_generic_5k.json")
    output_dir = os.path.join(workspace_dir, "qwen_binary_generator_lora_generic")
    
    print(f"Loading dataset from: {dataset_path}")
    if not os.path.exists(dataset_path):
        raise FileNotFoundError(f"Dataset not found at {dataset_path}. Please run generate_large_generic_dataset.py first.")
    ...
    training_args = SFTConfig(
        ...
        learning_rate=3e-4,
        logging_steps=50,
        num_train_epochs=10,            # Overfit training up to 10 epochs
        warmup_ratio=0.03,
        lr_scheduler_type="cosine",
        save_strategy="steps",          # Save checkpoints after steps
